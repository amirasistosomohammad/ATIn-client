...
export default function Settings() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      try {
        const res = await apiRequest('/settings', { method: 'GET', auth: false, cache: 'no-store' })
        setSystemSettings({
  ...
  const handleSystemSubmit = async (e) => {
    ...
    try {
      const res = await apiRequest('/admin/settings', {
        method: 'PUT',
        auth: true,
        body: { app_name: systemForm.app_name.trim() },
      })
      ...
      window.dispatchEvent(new CustomEvent('atin-settings-updated'))
  ...
  const handleAuthBackgroundChange = async (e) => {
    const file = e.target.files?.[0]
    if (!file) return
    if (file.size > 5 * 1024 * 1024) {
      showToast.error('Image must be 5MB or less.')
      return
    }
    ...
    try {
      ...
      const res = await apiRequestFormData('/admin/settings/auth-background', {
        method: 'POST',
        formData: fd,
        auth: true,
      })
  ...
  const fetchBackupList = useCallback(
    async ({ silent = false } = {}) => {
  ...
  const handleDownloadBackup = async () => {
    ...
    try {
      ...
      const filename =
        match?.[1]?.trim() ||
        `atin-backup-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.sql`
  ...
  const handleDownloadLatestBackup = async () => {
    ...
    try {
      ...
      const disposition = res.headers.get('Content-Disposition')
      const match = disposition?.match(/filename="?([^";\n]+)"?/)
      const filename = match?.[1]?.trim() || 'atin-backup-latest.sql'
      const blob = await res.blob()
      const href = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = href
            ...
            {activeTab === 'password' && (
              <div className="system-settings-tab-panel tab-transition-enter">
                ...
                <div className="system-settings-admin-note">
                  <strong>Administrator Note:</strong> As a system administrator, you can only change your password. Personal information modifications are restricted for security reasons.
                </div>
                  ...
                  <div className="system-settings-backup-table-wrap">
                    ...
                    {backupListLoading ? (
                      ...
                      <div className="system-settings-backup-table-empty">
                        No automated backups yet. Enable a schedule above and wait for the first
                        run, or use &quot;Download SQL backup now&quot; for a manual backup.
                      </div>
                    ) : (
